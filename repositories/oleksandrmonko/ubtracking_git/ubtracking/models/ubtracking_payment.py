...
class UBTrackingPayment(models.Model):
    ...
    def _compute_name(self):
        for record in self:
            if record.service_name and record.billing_period and record.payment_date and record.amount is not None:
                date_str = fields.Date.to_string(record.billing_period)
                record.name = f'Payment for {date_str} for {record.service_name}. Amount {record.amount:.2f}. Dated {record.payment_date}'
            elif record.billing_period and record.payment_date and record.amount is not None:
                date_str = fields.Date.to_string(record.billing_period)
                record.name = f'Payment for {date_str}. Amount {record.amount:.2f}. Dated {record.payment_date}'
            else:
                record.name = 'Billing Period, Amount, and Payment Day must be filled'
